...
OUTPUT_DIR = WORKDIR / "condition-M-C-harness-v42"
...
def build_polish_prompt(raw_path, missing_items, original_prompt=""):
    """Build the polish prompt with YAML work order, sign-off, and original prompt for factual verification."""
    ...
    if original_prompt:
        prompt_section = f"""
ORIGINAL USER PROMPT (source of truth for colors, labels, layout):
---
{original_prompt}
---
FACTUAL CHECK: After applying the work order fixes, verify the HTML matches
the original prompt. If the prompt specifies colors (e.g. "Pro in blue"),
labels, or layout requirements that the HTML doesn't follow, fix those too.
The prompt is the source of truth for what the component should look like.
"""
...
4. Check the original prompt for any color/style requirements the HTML missed
5. After applying ALL fixes, output the complete HTML
