#!/usr/bin/env python3
"""V4.1 Split Pipeline — Two separate pi sessions with bash checklist between them.

Session 1: Raw generate from prompt only
Bash: Checklist — grep for missing production items
Session 2: Polish — reads raw HTML + dictionary + checklist, rewrites file

Each session is a completely independent pi process. No YAML workflow.
This solves the "polish node doesn't actually rewrite" problem.

Output to condition-K-harness-v41/
"""
...
OUTPUT_DIR = WORKDIR / "condition-K-harness-v41"
...
def run_checklist(html_path):
    ...
    checks = {
        ...
        "has_display_size": bool(re.search(r'font-size:\s*(3|3\.\d|4|4\.\d|5)\d*rem|font-size:\s*(48|5[0-9]|6[0-9]|7[0-9])px|clamp\(2\.5rem', text)),
        ...
        "has_clean_glow": not (("box-shadow" in text and "60px" in text) and ("::before" in text and "0.5" in text)),
...
def build_fixes_yaml(missing_items):
    ...
    fix_map = {
        ...
        "clean_glow": {"id": "CR-09", "css": "::before { opacity: 0.15; } /* no box-shadow glow on card face */", "instruction": "Reduce ::before glow opacity to 0.15 max. Remove any box-shadow glow from the card face/inner element. The card surface must be a clean solid background, glow only on outer edge behind the card."},
...
def run_one(comp_id, prompt_text):
    ...
    # ── BASH CHECKLIST ────────────────────────────────────────────────
    checks, missing = run_checklist(cwd_index)
