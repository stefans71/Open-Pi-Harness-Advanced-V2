...
│                                                                                 │
│  ┌──────────────────────────────────┐   ┌─────────────────────────────────────┐ │
│  │  Generation Server (GPU)         │   │  Embedding Server (CPU)             │ │
│  │  llama-server / Ollama / vLLM    │   │  llama-server                       │ │
│  │  port 11434                      │   │  port 8081                          │ │
│  │                                  │   │                                     │ │
│  │  OpenAI-compatible:              │   │  nomic-embed-text-v1.5 (140MB)      │ │
│  │  /v1/chat/completions            │   │  /v1/embeddings → 768-dim vectors   │ │
│  │                                  │   │                                     │ │
│  │  Model: e.g. Qwen3-32B on GPU   │   │  Used by: pi-memory only            │ │
│  └───────────────┬──────────────────┘   └──────────────────┬──────────────────┘ │
│                  │                                          │                    │
└──────────────────┼──────────────────────────────────────────┼────────────────────┘
...
│  │                         Extension API                                       │ │
│  │                                                                             │ │
│  │   Lifecycle Hooks          Commands              Tools                      │ │
│  │   ──────────────           ────────              ─────                      │ │
│  │   session_start            /remember             pi_remember               │ │
│  │   session_before_compact   /forget               (model-callable)           │ │
│  │   before_agent_start       /memories                                        │ │
│  │   input                    /orchestrate                                     │ │
│  │                            /skills, /skill                                  │ │
│  │                            /workflow                                         │ │
│  │                            /project                                         │ │
...
│       │          │              │                     │                          │
│  ┌────▼────┐ ┌───▼──────────┐ ┌▼──────────────┐ ┌────▼────────────────────────┐ │
│  │pi-memory│ │pi-orchestrator│ │  pi-skills    │ │       pi-workflows          │ │
│  │         │ │              │ │               │ │                             │ │
│  │ HOOKS:  │ │ HOOKS:       │ │ HOOKS:        │ │ HOOKS:                      │ │
│  │ session │ │ session_start│ │ session_start │ │ input (natural language      │ │
│  │ _start  │ │  (load roles)│ │  (scan skills)│ │   trigger detection)        │ │
│  │ before_ │ │              │ │ input         │ │                             │ │
│  │ compact │ │ COMMANDS:    │ │  (trigger     │ │ COMMANDS:                   │ │
│  │ before_ │ │ /orchestrate │ │   matching +  │ │ /workflow run <name>        │ │
│  │ agent_  │ │ /agents      │ │   complexity  │ │ /workflow list|status       │ │
│  │ start   │ │ /roles       │ │   detection)  │ │ /project new|add           │ │
│  │         │ │              │ │               │ │                             │ │
│  │ CMDS:   │ │ Spawns PI    │ │ workflow:     │ │ EXECUTOR:                   │ │
│  │ /remember│ │ subprocesses │ │ completed     │ │ prompt, bash, approval,     │ │
│  │ /forget │ │ sequentially │ │ event →       │ │ loop, cancel nodes          │ │
│  │ /memories│ │ (single GPU) │ │ SkillCreator  │ │                             │ │
│  │ /vram   │ │              │ │ auto-creates  │ │ DAG ordering (Kahn's algo)  │ │
│  │         │ │ ROLES:       │ │ new skills    │ │ depends_on + when conditions│ │
│  │ TOOL:   │ │ coder        │ │ from traces   │ │ Variable substitution       │ │
│  │ pi_     │ │ reviewer     │ │               │ │ fresh_context (newSession)  │ │
│  │ remember│ │ researcher   │ │ COMMANDS:     │ │ allowed_tools (setActive)   │ │
│  │         │ │ orchestrator │ │ /skills       │ │ expected_artifacts          │ │
│  └────┬────┘ └──────┬───────┘ │ /skill <name> │ │ Resume via workflow-state   │ │
│       │             │         │ /skill create │ │                             │ │
│       │             │         │ /skill curator│ │ EMITS:                      │ │
│       │             │         └───────┬───────┘ │ workflow:completed event    │ │
│       │             │                 │         │  → consumed by pi-skills    │ │
│       │             │                 │         └─────────────┬───────────────┘ │
│       │             │                 │                       │                  │
└───────┼─────────────┼─────────────────┼───────────────────────┼──────────────────┘
        │             │                 │                       │
...
│  │               │  │                                                            │
│  │ Prompt ──────►│  │         ┌──────────────────────────────────────────────┐    │
│  │ embed query   │  │         │              ORCHESTRATION FLOW              │    │
│  │   │           │  │         │                                              │    │
│  │   ▼           │  │         │  /orchestrate "task"                         │    │
│  │ cosine search │  │         │       │                                      │    │
│  │ SQLite-vec    │  │         │       ▼                                      │    │
│  │   │           │  │         │  Task Analyzer (LLM call)                    │    │
│  │   ▼           │  │         │  simple → skip | complex → decompose        │    │
│  │ Inject into   │  │         │       │                                      │    │
│  │ system prompt │  │         │       ▼                                      │    │
│  │ [LONG-TERM    │  ◄─────┐   │  Plan + Role Assignment                     │    │
│  │  MEMORY]      │  │     │   │  (coder, reviewer, researcher)              │    │
│  │               │  │     │   │       │                                      │    │
│  │ Compaction ──►│  │     │   │       ▼  User confirms                      │    │
│  │ extract facts │  │     │   │  Sequential PI Subprocesses                  │    │
│  │   │           │  │     │   │  pi --json --no-input --system-prompt-file   │    │
│  │   ▼           │  │     │   │       │         │         │                  │    │
│  │ embed + store │  │     │   │  [Step 1]   [Step 2]  [Step N]              │    │
│  │ deduplicate   │  │     │   │  (coder)   (reviewer) (researcher)          │    │
│  │ (>0.95 merge) │  │     │   │       │         │         │                  │    │
│  └───────────────┘  │     │   │       ▼─────────▼─────────▼                  │    │
│                     │     │   │  Result Synthesizer (LLM merge)              │    │
│                     │     │   └──────────────────────────────────────────────┘    │
│                     │     │                                                       │
│                     │     └────── same llama-server instance (sequential, not     │
│                     │            parallel — single GPU constraint)                │
│                     │                                                            │
│  ┌──────────────────▼──────────────────────────────────────────────────────────┐  │
│  │                        WORKFLOW EXECUTION                                    │ │
│  │                                                                              │ │
│  │  /workflow run code-task "build a REST API"                                  │ │
│  │       │                                                                      │ │
│  │       ▼                                                                      │ │
│  │  Load YAML ─► DAG sort (Kahn's) ─► Execute nodes sequentially               │ │
│  │                                          │                                   │ │
│  │       ┌──────────────────────────────────┼─────────────────────┐              │ │
│  │       │              │                   │          │          │              │ │
│  │   ┌───▼───┐    ┌─────▼─────┐    ┌───────▼───┐ ┌───▼───┐ ┌───▼────┐         │ │
│  │   │prompt │    │   bash    │    │ approval  │ │ loop  │ │ cancel │         │ │
│  │   │       │    │          │    │           │ │       │ │        │         │ │
│  │   │send to│    │shell cmd │    │confirm /  │ │repeat │ │abort   │         │ │
│  │   │LLM    │    │w/ timeout│    │reject gate│ │until  │ │workflow│         │ │
│  │   │       │    │          │    │           │ │done   │ │        │         │ │
│  │   │fresh_ │    │allow_    │    │on_reject: │ │max_   │ └────────┘         │ │
│  │   │context│    │failure   │    │cancel /   │ │itera- │                    │ │
│  │   │allowed│    │          │    │rollback / │ │tions  │                    │ │
│  │   │_tools │    │          │    │continue   │ │       │                    │ │
│  │   └───────┘    └──────────┘    └───────────┘ └───────┘                    │ │
│  │                                                                              │ │
│  │  Variables: $USER_MESSAGE, $ARTIFACTS_DIR, $<nodeId>.output                  │ │
│  │  Artifacts: .pi/workflow-artifacts/<name>-<timestamp>/events.jsonl            │ │
│  │  Resume: workflow-state.json tracks completed nodes                          │ │
│  └──────────────────────────────────────────────────────────────────────────────┘ │
│                                                                                   │
└───────────────────────────────────────────────────────────────────────────────────┘
...
│                                                                                   │
│  <project>/.pi/                                                                   │
│  ├── memory.db              SQLite + sqlite-vec (facts + 768-dim vectors)         │
│  ├── workflows/*.yaml       Workflow definitions (committed to git)               │
│  ├── skills/                Auto-generated SKILL.md files (gitignored)            │
│  └── workflow-artifacts/    Per-run output: events.jsonl + workflow-state.json    │
│                                                                                   │
│  ~/.pi/agent/                                                                     │
│  ├── extensions/            Symlinks → stable clone (global, shared)              │
│  ├── models.json            Model endpoint config                                │
│  └── settings.json          Token limits, preferences                            │
└───────────────────────────────────────────────────────────────────────────────────┘
```
...
**Cross-extension event:** When pi-workflows completes a run, it emits a `workflow:completed` event. pi-skills listens for this and feeds the run's `events.jsonl` trace to `SkillCreator`, which auto-generates a new SKILL.md file — closing the self-improvement loop.
